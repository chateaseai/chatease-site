[12:03, 1/20/2026] Andreas: import React from 'react';
...
…
[12:05, 1/20/2026] Andreas: import React from 'react';
import { Link } from 'react-router-dom';
import ChatDemo from '../ui/ChatDemo.jsx';
...
export default function Home() {
  return (
    <section className="relative overflow-hidden">
      {/* Animated gradient backdrop (behind everything) */}
      <div
        className="pointer-events-none absolute inset-0 -z-10 animate-gradient opacity-15"
        aria-hidden="true"
      />

      {/* Soft overlay (reduced so it doesn't wash out the UI) */}
      <div
        className="pointer-events-none absolute inset-0 -z-10 bg-[radial-gradient(ellipse_at_top,_rgba(255,255,255,0.35),_transparent_60%)]"
        aria-hidden="true"
      />
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 py-20 md:py-28">
        <div className="grid gap-10 md:grid-cols-2 md:items-center">
          {/* LEFT: headline & CTAs */}
          <div className="text-center md:text-left">
            <span className="inline-flex items-center gap-2 rounded-full border px-3 py-1 text-xs md:text-sm bg-white/80 backdrop-blur">
            ...
            <div className="mt-7 flex justify-center md:justify-start gap-3">
              ...
              {/* Secondary CTA (solid white so it doesn't blend) */}
              <Link
                to="/services"
                className="rounded-lg px-5 py-3 border border-slate-300 bg-white hover:bg-slate-50 transition"
              >
                Explore services
              </Link>
            </div>
            ...
            <div className="mt-8 flex flex-wrap gap-4 text-sm justify-center md:justify-start text-slate-600">
              <Link to="/pricing" className="underline hover:text-slate-800">
                See pricing
              </Link>
              <span>•</span>
              <Link to="/about" className="underline hover:text-slate-800">
                About
              </Link>
              <span>•</span>
              <Link to="/contact" className="underline hover:text-slate-800">
                Contact
              </Link>
            </div>
          </div>
...
function Badge({ children }) {
  return (
    <div className="rounded-xl border bg-white p-4 text-center text-sm text-slate-700 shadow-sm">
      {children}
    </div>
  );
}
